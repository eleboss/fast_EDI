...
public:
  ...
  FlatBufferBuilder(FlatBufferBuilder &other)
	...
	uoffset_t EndTable(uoffset_t start) {
		...
		WriteScalar(
			buf_.data_at(vtableoffsetloc), static_cast<soffset_t>(vt_use) - static_cast<soffset_t>(vtableoffsetloc));
...
protected:
	...
	void Finish(uoffset_t root, const char *file_identifier, bool size_prefix) {
		...
		PreAlign(
			(size_prefix ? sizeof(uoffset_t) : 0) + sizeof(uoffset_t) + (file_identifier ? kFileIdentifierLength : 0),
			minalign_);
		if (file_identifier) {
